...
async fn test_play_endpoint_returns_next_move() {
    ...
    let coords = json.get("coords").unwrap();
...
async fn test_play_endpoint_rust_module_unreachable() {
    ...
    // Request with invalid bot_id to trigger error path
    let yen = YEN::new(4, 0, vec!['B', 'R'], "./../.../....".to_string());
    let json = json5::to_string(&yen).unwrap();
    let position = urlencoding::encode(&json);

    let query = format!(
        "/play?position={}&bot_id=nonexistent_bot&api_version=v1",
        position
    );
...
async fn test_play_endpoint_invalid_json_from_rust_module() {
    ...
    // Request with invalid position format to trigger json5 parse error
    let invalid_position = urlencoding::encode("{ invalid json }");
...
async fn test_play_endpoint_occupied_cell() {
    ...
    // Create a game where position (2,0,0) is occupied and all bots will try to place there
    // Since bots choose valid moves, this test may return a valid move or error depending on bot
    ...
    let bytes = response.into_body().collect().await.unwrap().to_bytes();
    let json: Value = serde_json::from_slice(&bytes).unwrap();
    // Either returns coords or error message
    assert!(json.get("coords").is_some() || json.get("message").is_some());
}
...
async fn test_play_endpoint_body_read_error_tcp_cut() {
    ...
    // Valid request - should succeed now that we don't make HTTP calls
    let yen = YEN::new(4, 0, vec!['B', 'R'], "./../.../....".to_string());
    let json = json5::to_string(&yen).unwrap();
    let position = urlencoding::encode(&json);
    ...
    // Should return coords since we now call bots directly
...
async fn test_play_uses_default_api_version_and_default_bot_id() {
    ...
    // Crea un YEN válido para enviar como position
    ...
    // Debe responder OK y devolver coords
    assert_eq!(response.status(), StatusCode::OK);
    let bytes = response.into_body().collect().await.unwrap().to_bytes();
    let json: Value = serde_json::from_slice(&bytes).unwrap();
    assert!(json.get("coords").is_some());
